...
async def faq_subscribe_callback(call: types.CallbackQuery):
    ...
    await call.message.answer(
        text="""Добро пожаловать в центр управления подпиской.

Подписка в фотобудке создана для того, чтобы на самых выгодных условиях иметь доступ к функциям создания контента, улучшения качества и оживления фото 📸

Сервис заменяет фотографов и дизайнеров, экономит десятки тысяч рублей!

Также с помощью фотобудки можно зарабатывать, оказывая услуги нейро-фотографа.

Статус подписки: 🟢 Оформлена""",
        reply_markup=builder.as_markup(),
        parse_mode="HTML"
    )
...
async def drop_subscribe_1_callback(call: types.CallbackQuery):
    ...
    builder.adjust(1, 1, 1)
    await call.message.answer(
        text='''Вы точно уверены, что хотите отменить подписку?

Если у вас возникли проблемы с качеством, напишите в наш VIP-отдел — мы оперативно вам поможем.
...
async def drop_subscribe_2_callback(call: types.CallbackQuery):
    ...
    await call.message.answer(
        text="""acefel, ваша подписка успешно отменена
