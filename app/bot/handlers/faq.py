...
async def get_faq(message: types.Message):
    ...
    await message.answer("""Пользоваться Пингвин ИИ супер просто! 🚀

1. Оплачиваете доступ.
2. Загружаете от 10 до 20 своих фото. Мы отправим подробную инструкцию, какие фото лучше выбрать.
3. Ждёте всего 5 минут и можете приступать к генерации фотореалистичных изображений! ✨

Мы предоставим вам множество уже настроенных стилей.

Вам останется только выбрать стиль и нажать одну кнопку!⚡️

В среднем 10 картинок создаются всего за 30 секунд!""", parse_mode="HTML", reply_markup=builder.as_markup())
...
async def faq_2_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        text="""Пользоваться Fotobudka AI супер просто! 🚀
...
В среднем 10 картинок создаются всего за 30 секунд!""",
        parse_mode="HTML",
        reply_markup=builder.as_markup()
    )
...
1. Мгновенная генерация супер реалистичных фото с вами в любом образе. Забудьте об изматывающих фотосессиях и услугах фотографа! 📸 

2. Создание ленты мечты в социальных сетях. Сохранили в закладки фото, которые хотели бы повторить? Fotobudka AI поможет создать всю вашу ленту, а вы при этом даже не подниметесь с дивана! ✨

3. Подготовка деловых портретных фото для решения важных задач. Идеально подойдет, чтобы отправить боссу или прикрепить к резюме. 👔

4. Возможность почувствовать себя внутри образа, в который в обычной жизни сложно попасть. Например, стать героем фильма Джеймса Кэмерона или выйти в стартовом составе любимого футбольного клуба плечом к плечу с кумирами.

И это только часть возможностей Fotobudka AI. Проверьте сами! 🌟""",
        parse_mode="HTML",
        reply_markup=builder.as_markup()
    )
